...
import { Barrier, disposableTimeout, isThenable } from 'vs/base/common/async';
...
export class ExtHostTesting implements ExtHostTestingShape {
	...
	public async $runTestsForProvider(req: RunTestForProviderRequest, token: CancellationToken): Promise<void> {
		const controller = this.controllers.get(req.tests[0].src.provider);
		if (!controller) {
			return;
		}
...
class TestRunQueue {
	...
	public enqueueRun(opts: {
		...
		const runner = () => {
			...
			record!.current = {
				publicReq: opts.req,
				factory: name => {
					const task = new TestRunTask(name, tasks.length, opts.dto, shared, this.proxy);
					tasks.push(task);
			...
			this.invokeRunner(opts.extensionId, opts.dto.id, opts.doRun, tasks);
		};
	...
	public createTestRunTask<T>(extensionId: string, request: vscode.TestRunRequest<T>, name: string | undefined, persist: boolean): vscode.TestRunTask<T> {
		...
		const task = new TestRunTask(name, 0, dto, new Set(), this.proxy);
		...
		task.onEnd.wait().then(() => this.proxy.$finishedTestRun(dto.id));
		return task;
	}

	private invokeRunner<T>(extensionId: string, runId: string, fn: () => Thenable<void> | void, tasks: TestRunTask<T>[]) {
		try {
			const res = fn();
			if (isThenable(res)) {
				res
				...
				this.handleInvokeResult(extensionId, runId, tasks, undefined);
			}
		} catch (e) {
			this.handleInvokeResult(extensionId, runId, tasks, e);
		}
	}

	private handleInvokeResult<T>(extensionId: string, runId: string, tasks: TestRunTask<T>[], error?: Error) {
		...
		Promise.all(tasks.map(t => t.onEnd.wait())).then(() => {
			this.proxy.$finishedTestRun(runId);
		});
	}
}
...
class TestRunTask<T> implements vscode.TestRunTask<T> {
	...
	constructor(
		...
		this.#taskId = taskId;
		proxy.$startedTestRunTask(dto.id, { id: taskId, name, running: true });
	}
	...
	private ensureTestIsKnown(test: vscode.TestItem<T>) {
		...
		while (true) {
			...
			if (sent.has(test.id) || !test.parent) {
				break;
			}
