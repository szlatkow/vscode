	...
	public async provideTextContent(resource: URI): Promise<ITextModel | null> {
		...
		switch (parsed.type) {
			case TestUriType.ResultActualOutput:
				text = test.tasks[parsed.taskId].messages[parsed.messageIndex]?.actualOutput;
				break;
			case TestUriType.ResultExpectedOutput:
				text = test.tasks[parsed.taskId].messages[parsed.messageIndex]?.expectedOutput;
				break;
			case TestUriType.ResultMessage:
				text = test.tasks[parsed.taskId].messages[parsed.messageIndex]?.message.toString();
				break;
		}
