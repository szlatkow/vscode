...
export const parseTestUri = (uri: URI): ParsedTestUri | undefined => {
	...
	if (request[0] === TestUriParts.Messages) {
		...
		const taskId = Number(request[2]);
		const part = request[3];
		const testExtId = uri.query;
		if (type === TestUriParts.Results) {
			switch (part) {
				case TestUriParts.Text:
					return { resultId: locationId, taskId, testExtId, messageIndex: index, type: TestUriType.ResultMessage };
				case TestUriParts.ActualOutput:
					return { resultId: locationId, taskId, testExtId, messageIndex: index, type: TestUriType.ResultActualOutput };
				case TestUriParts.ExpectedOutput:
					return { resultId: locationId, taskId, testExtId, messageIndex: index, type: TestUriType.ResultExpectedOutput };
			}
		}
	}
...
export const buildTestUri = (parsed: ParsedTestUri): URI => {
	...
	switch (parsed.type) {
		case TestUriType.ResultActualOutput:
			return msgRef(parsed.resultId, parsed.taskId, parsed.messageIndex, TestUriParts.ActualOutput);
		case TestUriType.ResultExpectedOutput:
			return msgRef(parsed.resultId, parsed.taskId, parsed.messageIndex, TestUriParts.ExpectedOutput);
		case TestUriType.ResultMessage:
			return msgRef(parsed.resultId, parsed.taskId, parsed.messageIndex, TestUriParts.Text);
		default:
